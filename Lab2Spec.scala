...
class Lab2Spec(lab2: Lab2Like) extends FlatSpec {
  ...
  it should "add non-numeric strings" in {
    val e1 = N(3)
    val e2 = S("moo")
    val e3 = eval(Binary(Plus, e1, e2))
    assert(e3 === S("3moo"))
  ...
  it should "divide a num by 0 and return infinity" in {
    val e1 = N(4)
    val e2 = N(0)
    val e3 = eval(Binary(Div, e1, e2))
  ...
  "Lt" should "return true if the first expression is less than the second" in {
    ...
    val e4 = N(7)
    val e5 = Undefined
    val e6 = eval(Binary(Lt, e4, e5))
    assert(e6 === B(false))

    val e7 = B(false)
    val e8 = S("4")
    val e9 = eval(Binary(Lt, e7, e8))
    assert(e9 === B(true))
  } 
